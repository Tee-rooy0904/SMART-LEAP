TrainingUI.mountPage({
  async init(ui) {
    ...
    document.getElementById('pdo-attendance-body').innerHTML = invitees.map((row) => `<tr><td>${ui.escapeHtml(row.user?.name || '--')}</td><td>${ui.escapeHtml(row.batchGroupNumber ? `Group ${row.batchGroupNumber}` : '--')}</td><td>${ui.statusChip(row.status || 'Not Marked', row.status === 'Completed' || row.status === 'Attended' ? 'attendance-present' : 'blocked')}</td><td>${ui.escapeHtml(ui.formatDateTime(row.checkedInAt || ''))}</td><td>${ui.escapeHtml(row.remarks || '--')}</td><td><div class="table-row-actions"><button class="btn-ghost" type="button" data-status="Attended" data-invitee-id="${row.id}">Mark Present</button><button class="btn-ghost" type="button" data-status="Missed" data-invitee-id="${row.id}">Mark Absent</button><button class="btn-ghost" type="button" data-status="Notified" data-invitee-id="${row.id}">Mark Late</button></div></td></tr>`).join('');
    document.getElementById('pdo-completion-body').innerHTML = invitees.map((row) => `<tr><td>${ui.escapeHtml(row.user?.name || '--')}</td><td>${ui.statusChip(row.status || 'Not Marked', row.status === 'Completed' || row.status === 'Attended' ? 'attendance-present' : 'blocked')}</td><td>${ui.escapeHtml((detail.program?.seminarFormCodes || []).length ? `0 / ${detail.program.seminarFormCodes.length} submitted` : '--')}</td><td>${ui.statusChip(row.status === 'Completed' ? 'Completed' : 'Incomplete', row.status === 'Completed' ? 'completion-done' : 'blocked')}</td><td><button class="btn-ghost" type="button" data-status="Completed" data-invitee-id="${row.id}">Mark Completed</button></td></tr>`).join('');
